...
// Global array to track all games in this session (across multiple batches)
var sessionGameIds = []
...
$('#newGameBtn').off('click').on('click', function () {
    ...
    // Reset chart images, but the report section remains available if it has data
...
function saveGameLog() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            // Save game ID to session tracking for reporting
            var gameId = response.filename.split('_').pop().replace('.json', '')
            sessionGameIds.push(gameId)
        },
        error: function (error) {
...
$('#refreshReportsBtn').on('click', function () {
    ...
    // Prepare data to send: list of game IDs from current session
    var requestData = {}
    if (sessionGameIds.length > 0) {
        requestData.game_ids = sessionGameIds
    }

    $.ajax({
        url: '/api/regenerate_comparison',
        type: 'POST',
        contentType: 'application/json', // Ensure JSON content type is sent
