...
def regenerate_comparison():
    ...
    try:
        ...
        from generate_report import generate_comparison_charts, generate_markdown_table
        import numpy as np

        request_data = request.json or {}
        filter_game_ids = request_data.get('game_ids', [])
        ...
        for log_entry in logs:
            ...
            # Extract game_id from filename or log_entry if possible.
            # Filename format: game_TIMESTAMP_GAMEID.json
            game_id = filename.split('_')[-1].replace('.json', '')
            
            if filter_game_ids and game_id not in filter_game_ids:
                continue
        ...
        def calc_metrics(stats_dict):
            ...
            if n_games == 0:
                # Return empty metrics if no games found
                return {
                    'config': {'depth': 3, 'n_games': 0, 'rollout_count': 10},
                    'metrics': {
                        'win_rate': 0, 'avg_move_time': 0, 'std_move_time': 0,
                        'avg_cp_loss': 0, 'move_match_rate': 0, 'total_moves': 0
                    }
                }
        ...
        generate_markdown_table(baseline_summary, hybrid_summary)
